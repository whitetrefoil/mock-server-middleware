...
function loadModule(modulePath) {
    try {
        ...
        if (overrides[modulePath] != null) {
            ...
            logger_1.default.log(chalk_1.green('Using Manual Override: ') + modulePath);
        }
        else {
            handler = requireNew(modulePath);
            logger_1.default.log(chalk_1.green('Using API definition: ') + modulePath);
        }
        ...
        if (e.code === 'MODULE_NOT_FOUND') {
            logger_1.default.warn(chalk_1.yellow('StubAPI not found: ') + modulePath);
            return convertJsonToHandler({ code: HTTP_NOT_FOUND, body: e });
        }
        logger_1.default.error(chalk_1.red('Errors in StubAPI: ') + modulePath);
...
function initialize(options) {
    ...
    logger_1.setLogLevel(config.logLevel);
}
